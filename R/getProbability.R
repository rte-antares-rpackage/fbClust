#' Classify the typical days by searching for external factors (e.g. climate factors) 
#' explaning the occurence of a particular typical day
#'
#' @param climate \code{data.table}, the first column contains the dates 
#' (header : Date, format : YYYY-MM-DD), 
#' other columns are external variable whose name must be given in the header. 
#' Data should be given with a daily time step.
#' @param cluster \code{data.table} output of \link{clusteringTypicalDays} or 
#' \link{clusterTypicalDaysForOneClass}
#' @param levelsProba \code{numeric or list}. if \code{numeric}, contains a vector 
#' of quantiles which will be used for all external variables and classes. 
#' if \code{list}, quantiles are described independenly for each class and each external variable (see examples below)
#' @param extrapolationNA \code{logical} should NA values of the matrix be 
#' extrapolated (\code{TRUE}) or not (\code{FALSE})
#' @param writeProba \code{logical} if you want to save the result in a rds file.
#' @param outputFile \code{character} a folder where the rds result is saved
#' 
#' @examples
#'
#' \dontrun{
  ...
  if(!extrapolationNA){
    ...
    if (writeProba) {
      outputFile <- paste0(outputFile, "/", gsub(":", "", gsub( " ", "_",as.character(Sys.time()))), 
                           "_MatProba_", paste0(unique(ClimQuantiles$class), collapse = "_"), 
                           ".rds")
      saveRDS(list(probaNotExtrapol, ClimQuantiles), outputFile)
    }
